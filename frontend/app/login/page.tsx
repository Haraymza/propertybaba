...
export default function LoginPage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    setError("");
    try {
      const normalizedPhone = toRequiredText("Phone", phone);
      const normalizedPassword = toRequiredText("Password", password);
      const loginRes = await authApi.login({ phone: normalizedPhone, password: normalizedPassword });
          ...
          <Card className="mx-auto w-full max-w-md border border-[var(--border)] bg-[var(--surface)] shadow-[0_10px_30px_rgba(15,23,42,0.08)]">
            <CardHeader className="px-5 pt-5">
              <div className="mb-2 flex justify-center lg:justify-start">
                <Image src={LogoPB} alt="Property Baba logo" width={64} height={64} className="rounded-xl ring-1 ring-[var(--border)] object-contain" />
              </div>
            ...
            <CardContent className="px-5 pb-5">
              <form onSubmit={onSubmit} className="space-y-4">
                ...
                <p className="text-sm text-[var(--muted)]">
                  New user? Contact us on WhatsApp to create your account.
                </p>
              </form>
            </CardContent>
          </Card>
